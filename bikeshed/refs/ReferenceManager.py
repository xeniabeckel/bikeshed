...
class ReferenceManager:
    ...
    def __init__(self, defaultStatus: t.Optional[str]=None, fileRequester: t.Optional[retrieve.DataFileRequester]=None, testing: bool=False):
    ...
    def fetchHeading(self, spec: str, id: str, status: t.Optional[str] = None, el: t.Optional[t.ElementT] = None) -> t.Optional[t.Dict[str, str]]:
    ...
    def getBiblioRef(
        ...
        if isinstance(bib, biblio.AliasBiblioEntry):
            ...
            if allowObsolete:
                ...
                if newBib is None:
                    if not quiet:
                        m.die(f"[{bib.linkText}] claims to be obsoleted by [{bib.obsoletedBy}], which doesn't exist. Either change the reference, of use [{bib.linkText} obsolete] to ignore the obsoletion chain.")
                    return None
                if not quiet:
                    m.die(
